...
const Topbar = () => {
  ...
  return (
    ...
    (<header className='p-4 flex items-center justify-between bg-dark sticky top-0'>
            ...
            <div className='p-2 px-4 rounded-full bg-black flex items-center gap-2'>
                <figure className='p-2 rounded-full bg-dark'>
                {
                    (currentSession && currentSession.user.image) ? 
                        <Image className='w-5 h-5' src={currentSession.user.image} alt="user profile pic" width={20} height={20} /> 
                    :
                        <UserIcon className='h-5 w-5' />
                }
                </figure>
                <p className='text-sm cursor-pointer hover:text-underline' onClick={signOut}>Logout</p>
            </div>
    </header>)
  )
}
